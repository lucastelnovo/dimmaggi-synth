...
export class OscillatorsPage {
  ...
  async changeFreqButton(osc) {
    let valueFreq = null;

    switch (osc) {
      case 1:
        valueFreq = this.osc1FreqFine;
        break;
      case 2:
        valueFreq = this.osc2FreqFine;
        break;
      case 3:
        valueFreq = this.osc3FreqFine;
        break;
      default:
        break;
    }
    ...
    const alert = await this.alertController.create({
      ...
      header: "Ingrese una frecuencia",
      inputs: [
        {
          type: "number",
          value: valueFreq,
          placeholder: "Frecuencia OSC " + osc,
          min: 20,
          max: 20000,
        },
      ],
      buttons: [
        ...
        },
        {
          ...
          handler: (value) => {
            ...
            if (!value[0] || value[0] < 20 || value[0] > 20000) {
              return;
            }

            switch (osc) {
              case 1:
                this.osc1Freq = value[0];
                break;
              case 2:
                this.osc2Freq = value[0];
                break;
              case 3:
                this.osc3Freq = value[0];
                break;
  ...
  createSampler(uri) {
    ...
    this.nativeAudio
      .preloadSimple("uniqueId1", uri.toString())
      .then(() => this.nativeAudio.loop("uniqueId1"));
